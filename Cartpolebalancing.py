...
def cartpole_balance(RecedingHorizon=True, sigma=0.0, time_horizon_length=30):
    ...
    for i in range(N):
        ...
        u[i+1] += np.random.normal(0.0, scale=sigma, size=None)
    ...
    #pickle.dump(x, open(f"Trajectories/xs_MPC_{time_horizon_length}_{sigma}_{N}.pkl", "wb"))
    #pickle.dump(u, open(f"Trajectories/us_MPC_{time_horizon_length}_{sigma}_{N}.pkl", "wb"))
